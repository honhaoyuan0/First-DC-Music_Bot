...
async def start_lavalink(event: hikari.ShardReadyEvent) -> None:
    ...
    builder = (
        ...
        lavasnek_rs.LavalinkBuilder(event.my_user.id, os.getenv("DISCORD_TOKEN"))
...
@plugin.command()
@lightbulb.add_checks(lightbulb.guild_only)
@lightbulb.command('loop', "Loop the current song thats allowed by HHY IF NOT SKIPP")
...
async def loop(ctx: lightbulb.Context) -> None:
    ...
    if node:
        if not node.now_playing:
            await ctx.respond("You need to play a song first in order to loop it")
        else:
        ...
        await ctx.respond("You need to be connected to a voice channel")


@plugin.command()
@lightbulb.add_checks(lightbulb.guild_only)
@lightbulb.command('sloop', "Stop looping the current song")
@lightbulb.implements(lightbulb.SlashCommand)
async def sloop(ctx: lightbulb.Context) -> None:
    node = await plugin.bot.d.lavalink.get_guild_node(ctx.guild_id)
    if node:
        if not node.now_playing:
